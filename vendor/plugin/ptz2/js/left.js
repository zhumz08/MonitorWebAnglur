...
function initRealTree()
{
	...
	realTree.attachEvent("onDblClick", function (nodeId)
	{
		...
		if (nodeId)
		{
			...
			// 重新加载点
			//[document.frames("refreshDeviceInMap").document.location="index.htmlr";
			//document.getElementById("refreshDeviceInMap").src="index.html";			
			// 重新加载点
			//var url = "http://15.32.12.224:8080/btmap/index.html?camid=" + currCameraId;
			//document.getElementById("refreshDeviceInMap").src = url; 
		}
	});
	//添加单击事件  根据摄像机ID 获取Host信息
	realTree.attachEvent("onClick", function (nodeId)
	{
		...
		if (nodeId)
		{
			...
			log.debug("定位摄像头视频   摄像机ID：" + realTree.getItemText(nodeId));
			currCameraName = realTree.getItemText(nodeId);
			currCameraId =nodeId.substring(1, nodeId.length);
			//保存当前窗口绑定的摄像头信息
			//winInfoArr[currWinNo]=[currCameraId,currCameraName];
